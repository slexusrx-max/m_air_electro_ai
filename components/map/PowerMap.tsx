...
export function PowerMap({ plants }: { plants: PowerPlant[] }) {
  const mapRef = useRef<MapLibreMap | null>(null); const [filters, setFilters] = useState<PowerFilters>({}); const [selectedPlant, setSelectedPlant] = useState<PowerPlant | null>(null); const [region, setRegion] = useState<(typeof mockRegions)[number] | null>(null); const [mapFailed, setMapFailed] = useState(false);
  ...
  const handleReady = useCallback((map: MapLibreMap) => { mapRef.current = map; if (selectedPlant) selectPlant(selectedPlant); }, [selectPlant, selectedPlant]);
