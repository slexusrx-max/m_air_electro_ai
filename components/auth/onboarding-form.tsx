  ...
  return (
    <section className="glass-panel mx-auto w-full max-w-2xl rounded-[2rem] p-6 sm:p-8">
      ...
      <form action={formAction} className="mt-7 space-y-6">
        ...
        {step === 3 && <fieldset><legend className="text-lg font-semibold">{t["onboarding.language"]}</legend><div className="mt-3 grid grid-cols-2 gap-3">{["en", "uk"].map((code) => <label key={code} className="rounded-xl border border-white/20 p-3 has-[:checked]:border-lime-200"><input className="mr-2" type="radio" value={code} checked={language === code} onChange={() => setLanguage(code)} />{code === "uk" ? "Українська" : "English"}</label>)}</div></fieldset>}
