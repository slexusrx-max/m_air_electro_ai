...
2. Set `NEXT_PUBLIC_SITE_URL` to that HTTPS origin, without a path/query. It is the single source for canonical, OG, sitemap, robots, JSON-LD and contact origin checking. Before migration the Vercel production hostname environment variable is a fallback; no fixed deployment hostname is embedded in implementation. Local fallback is `http://localhost:3100`. Rebuild after public env changes.
...
4. Create and verify `contact@CUSTOM_DOMAIN`, `partnerships@CUSTOM_DOMAIN`, `privacy@CUSTOM_DOMAIN`. Set `NEXT_PUBLIC_CONTACT_EMAIL`, `NEXT_PUBLIC_PARTNERSHIPS_EMAIL`, `NEXT_PUBLIC_PRIVACY_EMAIL`, then `CONTACT_EMAIL_VERIFIED=true`. The app accepts verified mailboxes only on the canonical custom host (ignoring `www`). The example addresses here are instructions, not published contacts.
